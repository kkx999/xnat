...
本次为 XNAT v1.4.1 的兼容性增量更新，重点收口机器展示身份、充值订单安全和 Telegram 通知引导。
...
- 正式支持 v1.4.0 → v1.4.1 原地升级，并继续保留更早版本的既有 additive migration 路径
- 升级继续执行 SQLite quick_check、完整备份、健康检查与失败回滚；.env、用户、余额、订单、VPS、Host、套餐、端口、工单、充值和通知数据全部保留
- Host Agent v1.1.1 / Agent API v1 核心协议不变；Host 可同步本 Release 的 CLI 与 Release 元数据
...
GitHub 发布时请创建并真正 Publish Tag `v1.4.1` 的 Release，不要只保留 Draft；无版本安装器通过 `releases/latest` 识别最新正式版。
