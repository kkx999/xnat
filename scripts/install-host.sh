...
compute_mode_capacity(){
  local mode="$1" prefix total_min reserve min_pool warn_total recommended_total requires_kvm="false"
  local pool_mib pool_gb ok="true" reason=""
  case "$mode" in
    lxc) prefix="LXC"; total_min=4608; reserve=1024; min_pool=1; warn_total=5120; recommended_total=8192 ;;
    kvm) prefix="KVM"; total_min=6656; reserve=1536; min_pool=4; warn_total=8192; recommended_total=12288; requires_kvm="true" ;;
    hybrid) prefix="HYBRID"; total_min=6656; reserve=1536; min_pool=4; warn_total=8192; recommended_total=12288; requires_kvm="true" ;;
    *) die "未知虚拟化模式：${mode}" ;;
  esac
  ...
  printf -v "${prefix}_RECOMMENDED_TOTAL_MIB" '%s' "$recommended_total"
}
...
PY_RES
  ...
  if (( ROOT_TOTAL_MIB < RECOMMENDED_TOTAL_MIB )); then
