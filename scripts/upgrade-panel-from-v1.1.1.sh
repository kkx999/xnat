...
[[ "$CURRENT_VERSION" == "1.1.1" ]] || die "此脚本只用于 v1.1.1 → v1.2.0；当前检测到 v${CURRENT_VERSION:-unknown}"
[[ "$(tr -d '[:space:]' < "${REPO_ROOT}/VERSION")" == "1.2.0" ]] || die "当前源码包不是 XNAT v1.2.0"

echo "已确认升级路径：XNAT Panel v1.1.1 → v1.2.0"
echo "现有 .env、SQLite 数据库、用户/余额/订单/VPS/Host/支付/通知/公告数据将全部保留。"
echo "升级脚本会先创建可回滚备份，再执行 additive 数据库迁移、健康检查并更新 Panel 文件。"
