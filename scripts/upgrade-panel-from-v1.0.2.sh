...
[[ "$CURRENT_VERSION" == "1.0.2" ]] || die "此脚本只用于 v1.0.2 → v1.1.0；当前检测到 v${CURRENT_VERSION:-unknown}"
[[ "$(tr -d '[:space:]' < "${REPO_ROOT}/VERSION")" == "1.1.0" ]] || die "当前源码包不是 XNAT v1.1.0"

echo "已确认升级路径：XNAT Panel v1.0.2 → v1.1.0"
echo "现有 .env、SQLite 数据库、用户/余额/订单/VPS/Host/支付/通知数据将保留。"
echo "升级脚本会先创建可回滚备份，再执行 additive SQLite migration。"
echo
