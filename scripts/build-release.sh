...
- Mobile API：v1
- Host 重装后，管理员主动更新 Agent URL / Token 会重置旧 TLS TOFU 指纹，下次成功连接重新建立信任
- 删除服务器改为 Panel-only：删除流程永远不请求 Host Agent，并清理 Panel 端口映射、敏感连接信息与待执行任务
- 每台 VPS 新增 iOS 风格自动续费开关，到期前 24 小时余额充足时自动续费 30 天，并加入幂等扣费保护
- 重装 / 删除区域新增一键复制机器编号，同时保留手动粘贴确认
- 登录、注册、2FA、找回 / 重置密码统一为高级双栏认证 UI，并完成移动端单栏适配
- 自动续费、手动续费、复制编号、重装 / 删除操作统一尺寸、圆角、间距与响应式对齐
- v1.0.5 → v1.0.6 为正式 Panel 升级路径；Host Agent v1.0.4 无需重复更新

> Mobile API 仍为 v1，新增字段保持向后兼容。
